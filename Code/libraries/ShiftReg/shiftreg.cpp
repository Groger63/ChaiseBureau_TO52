...
ShiftReg::ShiftReg(int DS , int STCP, int SHCP, int regshifts) : pin_DS(DS) , pin_STCP(STCP), pin_SHCP(SHCP), nb_regshift(regshifts)
{
  pinMode(pin_DS, OUTPUT);
  pinMode(pin_STCP, OUTPUT);
  pinMode(pin_SHCP, OUTPUT);
  
  registers = new boolean[nb_regshift * 8] ;
...
{
  for(int i =(nb_regshift * 8) -1; i >=  0; i--){
     registers[i] = HIGH;
  }
  writeRegisters();
}
